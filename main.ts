...
const BACKUP_URL = "https://github.com/PremVPN/PremVPN/raw/refs/heads/main/Karing_1.2.18.2102_ios_2026-06-03-1741.backup.zip";
...
interface UserData {
  ...
  note: string;
  expireDate: string;
  devices: string[]; // массив ID устройств
}
...
<head>
    ...
    <style>
        ...
        .device-count { background: #e3f2fd; padding: 2px 6px; border-radius: 10px; font-size: 11px; }
    </style>
</head>
<body>
<div class="container">
    <h1>🚀 ${CONFIG.BRAND_NAME} — Панель управления</h1>
    <p class="note">⚡ Экономичный режим | 📱 Отслеживание устройств</p>
    ...
    <table>
        <thead>
            <tr><th>Токен</th><th>Статус</th><th>Заметка</th><th>Срок действия</th><th>Устройства</th><th>Действия</th></tr>
        </thead>
        <tbody id="users-body">
...
<script>
    ...
    function renderTable() {
        ...
        tbody.innerHTML = usersData.map(u => {
            ...
            return '<tr>' +
                ...
                '<td><span class="device-count">📱 ' + devicesCount + '</span></td>' +
                '<td>' +
    ...
    document.getElementById('tokenSelect').addEventListener('change', function() {
        ...
        document.getElementById('generatedLink').textContent = token ? 
            'karing://restore-backup?url=' + location.origin + '/config?token=' + token + '&device=' + Date.now() :
